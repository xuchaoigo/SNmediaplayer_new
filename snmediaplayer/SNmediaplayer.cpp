...
int SNmediaplayer::registerChannelInfo(string name, string value)
{
	//cout<<"add channel"<<channel_info<<endl;
	if(name == "Channel")
	{
		string channel_info = value;
		...
		{	
			...
			if (fend!=string::npos)
			{
				...
				cout<<" registerChannelInfo tmpuserchannelid is "<<tmpuserchannelid<<endl;
			}
		}

		userchannelid_list.push_back(tmpid);
	}
	else if(name == "Channel_Init")
	{
		sn_channel_list.clear();
		userchannelid_list.clear();
	}
	else
	{
		cout<<"unknow channel info "<<name<<endl;
	}
	return 0;
}
